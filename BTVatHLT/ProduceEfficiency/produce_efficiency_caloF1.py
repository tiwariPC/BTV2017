	...
	# FROM HLT_HT300PT30_QuadJet_75_60_45_40_TripeCSV_p07_v2
		...
		for i in range(0,event.caloJet_num-1) :
			if event.caloJet_offmatch[i] >= 0 :
				print "event[", i, "]"
                       		print "event.caloJet_num:         ", event.caloJet_num
                         	print "event.caloJet_offmatch[i]: ", event.caloJet_offmatch[i]
                         	print "event.offJet_num:          ", event.offJet_num
                         	print "event.offJet_csv[matched]: ", event.offJet_csv[ event.caloJet_offmatch[i] ]
                         	print "event.offJet_pt[matched]:  ", event.offJet_pt[  event.caloJet_offmatch[i] ] 
                         	h_csvbeforefilter.Fill(                   event.offJet_csv[ event.caloJet_offmatch[i] ] )
                         	h_csvbeforefilterlog.Fill( -math.log( 1 - event.offJet_csv[ event.caloJet_offmatch[i] ] ) )
                         	if event.caloJet_hltBTagCaloCSVp05Double[i] :
                             		print "pass trigger"
                             		print "CSV:        ", event.offJet_csv[event.caloJet_offmatch[i]]
                             		print "online pT:  ", event.caloJet_pt[i]
                             		print "online eta: ", event.caloJet_eta[i]
                             		h_csvafterfilter.Fill(                   event.offJet_csv[ event.caloJet_offmatch[i] ] )
                             		h_csvafterfilterlog.Fill( -math.log( 1 - event.offJet_csv[ event.caloJet_offmatch[i] ] ) )
